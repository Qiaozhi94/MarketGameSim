...
名义价值，而保证金占用本身又依赖杠杆档位，因此同一个担保比例在不同档位下对应不同的
保证金率。此前写的「5% ≈ 105% 的同一杠杆水平」和「互为倒数」都不成立，已删除——
**本表的公式是唯一口径，不要试图与旧数值互相印证**。
...
`reserved_units` 已包含当前持仓的保证金（代理策略 §11.1），再从权益里减一次就是
重复扣除——那会让合法订单被错误拒绝。

反例（equity = 1500、持仓 100 @100、10x）：旧式算得可用 `1500 − 1000 = 500`，
开到 140 手需 1400 > 500 被拒；而实际 `IM(140) = 1400 ≤ 1500`，本应通过。
