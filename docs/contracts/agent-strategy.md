...
从而凭空增强动量效应，而动量恰好是被检验的因子之一（001 / P-1）。该选择是模型
假设，须在报告中声明。
...
`w` 在运行开始时从 Dirichlet 分布抽取后**固定不变**（001 / D-3、FR-009）。抽取后
按 `Σ|wᵢ| = 1` 归一化。
...
做市商越薄，同样的强平量造成的价格滑落越深（001 / A-005）。
...
同一决策序号上的 `noise`」在两组中保持一致，与行为是否分叉无关（001 / KR-004）。
