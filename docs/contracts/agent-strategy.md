...
从而让「杠杆上限分布及其内生建仓后果是否足以产生连锁」成为可测量的问题（旗舰问题）。
**不写「初始杠杆分布」**——初始仓位恒为 0，初始杠杆也恒为 0（账户合同 §7）。
...
| 目标仓位线性映射信号 | 排除了阈值型、金字塔加仓等真实行为 | 0.1.3 替代行为映射 |
| 不自动拆单 | 大额调仓一次性冲击簿 | 大资金执行者引入时（v0.2+） |
| 永续资金费为 0 | 长时间持仓无成本，永续价格不锚定任何外部参照 | 实验时长显著延长时 |

**0.1.3 的稳健性检验必须至少替换其中一项**（PRD §15），以确认旗舰结论不依赖单一行为
