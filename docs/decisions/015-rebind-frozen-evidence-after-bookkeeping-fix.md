...
1. **「经济投影」是唯一判据，其定义在代码里**：
   ...
   **v2（2026-09-23）追加排除 `schema_version`**：日志**格式版本**不是经济事实，而它写在
   ...
   冻结摘要失配，哪怕一笔委托、一笔成交、一个决策都没变——本轮 v4 → v5（运行头新增
   `bootstrap_anchor`）就是第一例。排除它不会掩盖行为变化：真的行为变化仍然会落在其他
   字段上。
2. **允许重绑的条件（全部满足）**：
...
- 负面：新增一类维护对象（经济投影定义）；今后任何改变经济投影的 L1 改动都**不能**走
  本通道，只能重新执行实验。
- **格式升版与冻结证据的结构性张力**：`schema_version` 进入事件摘要，意味着每次升版都要
  重绑一次冻结证据。本 ADR 让这件事有正式通道且留痕，但没有消除张力；若升版变得频繁，
  应另行决策是否把格式版本移出摘要输入（那是改事件 Schema §7，需独立 ADR）。
- 后续行动：
