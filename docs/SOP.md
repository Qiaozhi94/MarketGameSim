...
- 单次运行不得作为一般性结论；样本量 1、不可复现、有学习效应的运行（`interactive`
  模式）不进任何统计。
  **作用域说明（2026-09-20 起，未闭合裁决）**：本条在**人类扰动实验**语境下的适用边界
  由 [`0.4.2` 的 `Q-601`](features/0.4/0.4.2-human-perturbation/spec.md#8-待确认问题)
  拥有——争点是分析单元为**市场运行（seed）**还是**人**：前者下同种子对照的效应量估计
  未必落入本条禁令，后者下必然落入。**裁决前本条按字面全额生效**：owner 自由会话的产物
  不得进入任何统计性结论，不得与 0.3.1 的 AI 研究声明合并，`0.4.2` 的效应量报告
  （`SC-603`）只能以 `experiment-preview` 形态呈现、不得声明研究结论。裁决落地时必须
  同步修订本条——只改 `0.4.2` 而留着本条不动，等于让两份文档对同一命题给出不同判断。
