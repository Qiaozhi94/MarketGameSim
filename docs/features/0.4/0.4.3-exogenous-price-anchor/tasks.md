...
- [ ] T1100 `[阻塞·须先裁决]`: **T1002 的处置裁决——「策略层违反自身契约」**
...
- [ ] T1102 (`Q-701`, `Q-702`): 冻结 `v_t` 的形态、参数与信息层归属；纯恒定形态已被
      0.4.1 §14 实测排除（价格钉死、异质性塌陷），随机过程被 ADR-017 排除，故候选只剩
      「确定性非随机路径」— verify: `tests/unit/metrics/`
- [ ] T1103 `[量级哨兵]` (`NFR-701`): 在族层仓位上限处加一条**量级哨兵**——计算出的上限
...
- [ ] T1111 (`AC-701`, `AC-702`): 运行 `v_t` 复现、价值族单调性与强度前置的正反测试
      — verify: `tests/unit/metrics/test_value_process.py`
- [ ] T1112 (`AC-703`, `AC-705`): 运行归因对照与锚过强检出的测试
      — verify: `tests/integration/test_attribution_control.py`
- [ ] T1113 (`AC-704`): 运行达标与未达标两条路径的门禁测试
      — verify: `tests/integration/test_market_quality_gate.py`
- [ ] T1114 (`AC-701`—`AC-705`): 运行项目统一质量门，确认 0.4.1 与 0.3.1 路径无回归
      — verify: `python tools/verify.py`
- [ ] T1115 `[状态门]`: 回写 spec 验收证据、版本索引与状态；必须是本文件最后一项
      — verify: `python tools/validate_spec_lifecycle.py`

## 4. 依赖与并行关系

- `T1100, T1101 -> T1102`：强度挂载点与 ADR 修订都未定时，参数冻结没有依据。
- `T1100` 与 `T1103` **同一轮落地**：两者都改 src，攒在一起走一次盖章（省一轮经济等价证明）。
- `T1102 -> T1104 -> T1105 -> T1106 -> T1107`：价值过程、族、强度前置、成果门依次串行。
- `T1107 -> T1108 [P]` 与 `T1107 -> T1109 [P]`：归因对照与锚过强检出互不共享状态。
