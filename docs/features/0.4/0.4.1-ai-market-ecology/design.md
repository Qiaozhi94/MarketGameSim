...
- 运行头新增字段：`bootstrap_anchor` 已落地（0.4.1 T963/T966，事件日志格式 v4 → v5，
  事件 Schema §2/§6.1）。`strategy_roster_id` **本轮不进运行头**：live 市场不写事件日志，
  清单标识随其 metrics artifact 落盘即可满足 DR-501 的追溯；而运行头每加一个字段就是一次
  格式升版，会让全部冻结证据摘要失配、触发一轮 ADR-015 重绑（v5 那次的实测代价见
  `docs/decisions/015-*.md`）。因此把它并到「首次由 roster 驱动、且要写事件日志的运行」
  那次升版里一起加——承接任务是 T967。字段增补遵循既有运行头合同的向后兼容规则，
  不改既有字段语义。
...
### 9.1 生态设计约束：稳定力的记忆不得短于最快的不稳定力视窗
...
### 9.2 排查方法：「算得出放大系数」≠「该约束 binding」

同一次排查连续否定了四个各自算术正确的假设（残单累积、MULT 缺失形成财富反馈、
卖方爆仓、上限改现金基准），并行会话另有一例（AC-509 把 `advance()` 次数当逻辑秒，
1 次 ≈ 1.55 秒）。共同形态是：**算出了一个放大系数、不对称或量纲关系，就当它在起作用，
没有先确认那个约束是否 binding、量纲是否成立**。

规则与手段：先测 binding，再谈机制；验证方式是内存补丁 + 逐位对比——
**改掉一个「关键约束」后若输出逐位不变，它就不是约束**。两次否定（MULT、现金基准）
都是这样在几分钟内完成的，比继续推演便宜得多。
