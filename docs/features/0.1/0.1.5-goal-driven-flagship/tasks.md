...
- [ ] **T200** `[成果门:R1]` (`FR-019`, `FR-020`): 为现有已验证管线增加单命令成果入口，
...
- [ ] **T207** `[成果门:R2]` (`AC-001`, `AC-002`, `AC-005`): 用固定种子生成目标驱动代理
...
- [ ] **T213** `[成果门:R3]` (`FR-024`, `AC-006`, `AC-007`): 用小种子计划生成四 cell ×
...
- [ ] **T216** `[成果门:R4]` (`FR-024`, `FR-026`, `AC-007`, `AC-009`): 交付三终点正式
      结果、效应量/不确定性、代表性回放与可复现 manifest；仅允许
      `SPONTANEOUS + formal-research` — verify: `docs/experiments/`

## 3. 验证与验收任务

- [ ] **T217** (`AC-001`—`AC-010`): 逐项核对退出条件、批量回归与正式证据路径 — verify: `python tools/verify.py`
- [ ] **T218** (`AC-001`—`AC-010`): 运行项目统一质量门和确定性复现 — verify: `python tools/verify.py`
- [ ] **T219** (`FR-026`, `AC-009`, `AC-010`): 回写验收证据并仅在正式证据成立后推进 `done / established` — verify: `tools/validate_spec_lifecycle.py`
- [ ] **T220** `[成果门:R5]` (`AC-009`, `AC-010`): 生成 v0.1 研究交付入口，确保非开发者从
