...
- 阶段成果门规则见 [`docs/features/README.md`](../README.md#阶段成果门)（唯一拥有者）：
  每个 Phase 的**最后一项**任务标记 `` `[成果门:<ID>]` ``，写明用户可打开的产物、生成
  入口、验收动作和证据标签。
...
- [ ] T005 `[成果门:R1]` (`AC-001`, `AC-002`): 生成 <可打开产物>，标记为
...
- [ ] T010 [TEST] (`AC-001`): <旅程场景>，先立红灯夹具、收尾全量跑 — verify: <E2E 命令/证据路径>

从体验旅程派生（每旅程步骤 ≥1 条可执行断言）；**编写早、执行晚**——夹具在里程碑早期以红灯立起，收尾全量执行作为验收。开工门禁（SDD Flow T3）要求本组存在，缺失将拒绝流转。

- [ ] T011 `[状态门]`: 回写 spec 验收证据、活跃索引和状态；必须是本文件最后一项 — verify: `<feature gate command>`
