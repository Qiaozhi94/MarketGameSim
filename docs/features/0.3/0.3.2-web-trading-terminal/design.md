...
- **owner 成果包固定布局**：`docs/experiments/owner-n-of-1/` 下 `environment.json`
  ...
  真实采集冻结/实现；采集后的 index 冻结（原 T942）随 N-of-1 轨归档（ADR-010）。
...
- **交互设计原型（最终交付版，可离线打开）**：[`interaction-design.html`](interaction-design.html) ——
  ...
  PERP/USDT 双单位下单、下单面板杠杆选择条（1/2/3/5× 快选 + 10× 行尾下拉）与可配初始资金
  （资产页）、挂单/回报全宽底栏、多周期 K 线、
  SIM 环境角标与「ⓘ 市场说明」。本版为 owner 2026-09-18 定稿的真实终端形态（承接 v4 反馈与
  Q-403/ADR-006 裁决；杠杆溢出档位为行尾下拉，采集白名单含无头负向门）；设计依据、逐项对齐
  审查、定稿记录与评审模式（`#review`）说明迁入
  [`interaction-design-notes.md`](interaction-design-notes.md)。
- 桌面优先三栏：左侧盘口与最近成交，中间 K 线与底部持仓/委托/回报，右侧下单面板；
  账户经下单面板可用行与资产页呈现。
...
- **采集模式观察字段白名单**（与 spec §5 一致）：`best_bid`/`best_ask`/`last`、市场时钟、
  ...
  保证金/保证金比率/账户状态、当前挂单与动作状态。定稿原型的 24h 涨跌/高低/成交量、标记
