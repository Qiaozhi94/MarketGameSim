...
|---|---|---|---|
| 控制设计（Q-308 已裁决） | 每个人类运行配两条同 seed/config 纯代理对照：主对照 `WINDOW_MATCHED_POLICY_CONTROL`、次要描述性 `GOAL_AGENT_CONTROL`；不把 v0.1 样本直接复用 | 主要对照匹配可观测接口与决策机会，但不声称目标函数相同；第二条对照不增加招募 | 预生成后以哈希锁定；主对照策略预注册且不为 H2 调参 |
...
| 非代码关键路径 | T902 冻结伦理/招募 owner、资源上限和目标日期；T917 使用结果盲招募台账 | 审查、招募与排期不能由 `verify.py` 保证 | no-go 时禁止冻结正式协议并重新评审是否继续技术 preview；正式采样后招募不足则保留非证据样本流但不得建立声明或关闭 v0.3 |
...
- Q-308 已定：主对照是与参与者共享可观测接口和决策窗口的参照策略代理，原方向性目标代理
  退为次要描述性对照；不声称参照策略目标函数与参与者实际目标对齐。
  Q-301 剩余待定的是两者各自取自哪个 v0.1 家族与参数实例。
- `WINDOW_MATCHED_POLICY_CONTROL` 与 `GOAL_AGENT_CONTROL` 的策略 ID 或参数组合必须不同；协议须
  记录主对照选择依据及其事前证据，禁止根据 H2 结果挑选或交换主、次标签。
...
- 建议先以 H1 输入和模拟人类策略估计 participant-seed 配对差值的方差，再做功效模拟；
  不凭经验人数直接冻结正式样本量。
- 功效模拟必须把 participant 作为聚类/重复测量单位，显式模拟参与者内相关性、pair 缺失、
  三个主要 estimand 与多重性；不得把“人数 × 每人局数”直接当作独立样本量。
...
- 在运行功效模拟前先冻结可招募人数、参与者小时和预算上限。若达到目标功效所需资源超过
  任一上限，T902 必须判定 no-go，不得冻结正式协议；只能在不查看正式数据的前提下重新设计
  场景/estimand，或重新评审是否继续仅标记为 `experiment-preview` 的技术实现与验证。
...
| 维度 | `HUMAN_REPLACEMENT` | `WINDOW_MATCHED_POLICY_CONTROL` | 合同 |
|---|---|---|---|
| 账户与风险边界 | 冻结目标插槽 | 同一目标插槽 | 相同 |
| 信息集与动作空间 | 冻结公开观察与订单接口 | 同一 schema 与权限 | 相同 |
| 决策机会 | 有限窗口、每窗一次、超时 `NO_ACTION` | 同一调度器与参数 | 相同 |
| 决策目标 | 参与者文字任务、激励；实际目标不可观测 | v0.1 策略 ID/version/参数与显式目标 | **不同且必须分别披露** |
