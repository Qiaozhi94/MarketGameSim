...
| 非代码关键路径 | T902 冻结伦理/招募 owner、资源上限和目标日期；T917 使用结果盲招募台账 | 审查、招募与排期不能由 `verify.py` 保证 | no-go 时只交付 preview 并重新评审；正式采样后招募不足则保留样本流但不得建立声明或关闭 v0.3 |
...
- 在运行功效模拟前先冻结可招募人数、参与者小时和预算上限。若达到目标功效所需资源超过
  ...
  场景/estimand，或将本轮限定为 `experiment-preview` 后重新评审版本范围。
