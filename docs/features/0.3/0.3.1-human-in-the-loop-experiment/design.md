...
- 全部正式场景固定同一非做市目标插槽、角色、初始账户、杠杆上限与风险参数；所有者不得
...
- `AI_FORMAL` 轨：168 个配对 block 按批处理执行，无墙钟、无休息约束，也没有会话时长上限。
- 训练阶段允许按预先写明的超时率判据把墙钟窗口从 8 秒调到 10 秒，且只允许调整一次；不得
  缩短逻辑窗口，也不得依据任何结果指标调整。调整后重新生成协议哈希，再开始正式采样。

#### Q-304：样本量、停止与多重性（已按双轨重基线）

- `AI_FORMAL` 轨固定收满 **168** 个合格配对 seed block，不按显著性提前停止。依据是三个
  家族各自的 0.25 SD SESOI：`price_crash` 与 `liquidity_dry_up` 为 `4.5969e-3`
  （[`H2-endpoint-calibration`](../../../experiments/H2-endpoint-calibration.md)），
  `liquidation_cascade` 为 `0.25049`
  （[`H2-cascade-calibration`](../../../experiments/H2-cascade-calibration.md)），
  下限校验见 [`H2-block-power-baseline`](../../../experiments/H2-block-power-baseline.md)。
- 主要 estimand 是严重程度配对差；三个家族的发生指标实测为零方差，只作描述性报告。
  Holm 校正只作用于 AI 轨的三个主要终点。
- `OWNER_N_OF_1` 轨固定完成 24 个正式配对场景，不因盈亏或结果方向补跑，结果全部描述性。
- 旧的真人功效计算、参与者小时与 `$5,900` 资金预算已作废，不得进入任何 go/no-go；它们
  只保留在标记 `SUPERSEDED_*` 的证据文件里。

#### Q-305：排除、补跑与中止

- 项目不招募外部真人，因此没有资格筛选、理解检查门或同意书流程；所有者完成 6 个训练
  场景后进入正式采样。
- 客户端断线但服务端正常时窗口照常推进并写 `NO_ACTION`，不补跑；只有服务端、日志或
