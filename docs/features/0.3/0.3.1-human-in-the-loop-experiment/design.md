...
- 所有者墙钟、训练和正式场景安排由 0.3.2 单独实现，并复用本节冻结的窗口接口。
  （2026-09-13 修订：owner 侧合同已由 0.3.2 Q-403/ADR-006 改为自由交易 + 时点采样，
  见 0.3.1 spec 修订记录；AI 轨窗口调度不变。）
...
1. **窗口调度匹配**：`WINDOW_MATCHED_POLICY_CONTROL` 形态的两条参照必须运行在与所有者相同的有限窗口调度器下
   ...
   owner 已由 Q-403/ADR-006 移除决策窗，不再要求参照与 owner 同窗。）
...
3. **匹配边界如实声明**：匹配账户/风险边界、信息集、动作空间与决策机会；两条策略的目标、
   ...
   Q-403/ADR-006 改为市价/限价/撤单 + 自由连续时间，必须作为两处已披露差异单独声明。）
...
| 信息集与动作空间 | 冻结公开观察与订单接口（市价/限价/撤单） | 同一 schema 与权限（参照仅市价，无挂单/撤单） | 信息集相同；**动作空间不再相同**（2026-09-13 修订：Q-403） |
| 决策机会 | 自由连续交易、无用户可见决策窗、服务端逻辑时点采样 | 同一调度器与参数（两条参照共享有限窗口） | **不再相同**（2026-09-13 修订：Q-403；owner 为真实连续时间，参照为有限窗口） |
