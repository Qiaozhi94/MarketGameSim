...
class TestSchemaVersionAndKinds:
    def test_schema_version_is_5(self, raw_schema):
        assert raw_schema["schema_version"] == 5

    def test_record_kinds_match(self, raw_schema):
        assert raw_schema["record_kinds"] == ["RUN_HEADER", "EVENT", "RUN_TRAILER"]

    def test_run_header_field_count_17(self, registry):
        assert len(registry.field_names("RUN_HEADER")) == 17
