...
""".format(project=os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    # Run with PYTHONHASHSEED=1
    env1 = {**os.environ, "PYTHONHASHSEED": "1"}
    r1 = subprocess.run([sys.executable, "-c", script], capture_output=True,
                        text=True, env=env1, cwd=os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    # Run with PYTHONHASHSEED=2
    env2 = {**os.environ, "PYTHONHASHSEED": "2"}
    r2 = subprocess.run([sys.executable, "-c", script], capture_output=True,
                        text=True, env=env2, cwd=os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    h1 = r1.stdout.strip()
    h2 = r2.stdout.strip()
