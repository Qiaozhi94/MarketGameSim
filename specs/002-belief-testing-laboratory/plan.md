...
| 存储 | 事件日志 JSONL（规范序列化，ADR-005 §7）；分析产物 Parquet |
...
（代理策略 §10.1），KR-002 的时间是整数纳秒，ADR-005 的金额是最小单位整数——
...
- L4 只消费事件日志，**不被 L1—L3 引用**（NFR-004、002 / D-7）；
- L2 通过接口与 L1 交互，不直接触碰订单簿内部结构；
- 制度钩子（002 / D-1）由 L1 定义接口、由配置注入实现，**撮合核心不含制度判断**。

### 2.1 模块与其约束合同

| 模块 | 职责 | 受约束于 |
|---|---|---|
| `kernel/` | 事件队列、全序键、KR-006 单调性断言 | 事件 Schema §1、ADR-006 |
| `book/` | 订单簿、撮合、自成交阻止 | [撮合合同](../../docs/contracts/matching.md) |
| `ledger/` | 账户、保证金、强平、穿仓核销 | [账户与保证金](../../docs/contracts/margin-and-account.md) |
| `eventlog/` | 事件写入、规范序列化、摘要哈希 | ADR-005 §7、事件 Schema §6—§9 |
| `config/` | 解析、单位换算、校验 | ADR-005 §2 |
...
**不引入 `Decimal` 到领域层**——`Decimal` 只出现在配置解析（ADR-005 §2）与因子
