# M1 任务拆解

**对应里程碑**：[`M1-minimal-kernel.md`](M1-minimal-kernel.md)  
**状态**：Ready（P0-I01/I02 已于 2026-08-01 关闭）

## 约定

- 每个任务标注 `[合同引用]`——实现前先读该节，实现后对照验收；
- **测试先行**：带 `[TDD]` 的任务，先写失败的测试再写实现；
- 任务顺序即依赖顺序，同一 Phase 内可并行的标 `[P]`。

---

## Phase 1：数值与配置基础

- [ ] **M1-T101** `[ADR-001 §1]` 定义整数值对象：`Price`、`Quantity`、`Cash`、`Bp`、
      `Nanos`。全部不可变，禁止浮点构造。
- [ ] **M1-T102** `[ADR-001 §2]` `[TDD]` 配置解析：YAML 领域量必须为**字符串**，
      经 `Decimal` 转最小单位整数。**收到 float 即报错**，不接受先转 str 的补救。
- [ ] **M1-T103** `[ADR-001 §2]` `[TDD]` 配置校验：
      `tick_size × min_quantity` 是 `cash_unit` 的整数倍；
      `latency_ns ≥ 1`（KR-006）；`leverage_tier_distribution` 各档之和 = 10000。
- [ ] **M1-T104** `[ADR-001 §7]` `[TDD]` 规范序列化：整数字面量、缺失值 `null`、
      UTF-8/NFC、`ensure_ascii=false`、`separators=(",",":")`、键按码位升序、
      每事件一个 LF。**断言两次序列化逐字节相同**。

## Phase 2：事件内核

- [ ] **M1-T201** `[事件 Schema §1]` 实现双键：队列事件使用
      `(timestamp, priority_class, enqueue_seq)`，日志记录使用
      `(timestamp, transaction_seq, record_index)`；三个计数器的作用域与分配时点不得混用。
- [ ] **M1-T202** `[事件 Schema §1.1]` `[TDD]` **KR-006 单调性断言**：入队时校验
      `queue_key(新事件) > queue_key(当前队列事件)`，违反则抛异常终止，**不得静默重排**。
- [ ] **M1-T203** `[事件 Schema §1.2]` `[TDD]` 回退跳转白名单：只有
      `AGENT_DECIDE→ORDER_ARRIVAL` 与 `MARGIN_CALL→ORDER_ARRIVAL` 可回退 class，
      且必须跨越 ≥ 1 ns。表外回退即缺陷。
- [ ] **M1-T204** `[事件 Schema §3]` 优先级类别枚举（含 `MARGIN_CALL` 同为 class 1）。
- [ ] **M1-T204b** `[事件 Schema §1.4]` `[TDD]` **队列事件 vs 事务记录**：只有
      `ORDER_ARRIVAL`/`AGENT_OBSERVE`/`AGENT_DECIDE`/`SNAPSHOT` 入队；
      `TRADE_SETTLE`/`MARGIN_CALL`/`MARKET_DATA_PUBLISH`/事务内 `CANCEL` 直接写日志。
      **验收用例**：两张同代理订单同时间戳到达，第一张成交耗尽保证金 →
      **第二张必须被拒**（若记录入队则会误放行）。
- [ ] **M1-T205** `[事件 Schema §6—§9]` `[P]` 事件日志写入器 + 运行元数据头部
      （含 `tick_size`/`min_quantity`/`cash_unit` 单位定义）。
- [ ] **M1-T206** `[事件 Schema §7]` `[TDD]` 事件摘要哈希：只纳入 E-002 声明的语义
      字段，**排除**因果外键与 `event_id`；在规范编码之上计算。

## Phase 3：订单簿与撮合

- [ ] **M1-T301** `[撮合 §1.1]` `[TDD]` 订单簿结构：买降序/卖升序，同价按到达事务
      `transaction_seq` 升序。
      **禁止依赖字典遍历顺序**。
- [ ] **M1-T302** `[撮合 §2.1]` `[TDD]` **成交价取 maker 挂单价**，非 taker 限价。
      用例：买单限价 101 吃卖价 100 → 成交于 100。
- [ ] **M1-T303** `[撮合 §2.2]` `[TDD]` **跨档拆分**：一张单吃多档 → 多个
      `TRADE_SETTLE`，`caused_by_event_id` 相同、共享 `transaction_seq`、`record_index` 递增、
      `valuation_mark` **逐笔推进**（不是整批共用）。
- [ ] **M1-T304** `[撮合 §3]` `[TDD]` 剩余处理：限价单挂入簿（**保留到达事务的
      `transaction_seq`**）；
      市价单按 IOC 全额撤销。
- [ ] **M1-T305** `[撮合 §4]` `[TDD]` 自成交阻止：cancel-resting，撤销簿上旧单并写
      `reject_reason=SELF_TRADE_PREVENTION`，taker 继续吃下一档、不消耗数量。
- [ ] **M1-T306** `[撮合 §5]` 准入与撮合的固定顺序：制度钩子 → 对齐检查 →
      （保证金检查桩，M1 恒通过）→ 撮合 → 剩余处理。
- [ ] **M1-T306b** `[撮合 §1.2]` `[TDD]` **撮合事务原子性**：`ORDER_ARRIVAL` 弹出时
      在单个事务内完成撮合、逐笔结算、剩余处理、风险检查；事务内账户变化立即生效。
      **验收用例**：一张大单跨三档 → 三笔 `TRADE_SETTLE`（`batch_index` 0/1/2、
      `batch_size` 3）+ **仅一次**批次风险检查。
- [ ] **M1-T307** `[撮合 §6]`、`[退化 §1]` `[TDD]` 空簿与单边簿：市价单 IOC 撤销、
      `mid` 未定义时 `valuation_mark` 退化为 `last`、首笔成交前退化为
      `initial_price`。
- [ ] **M1-T308** `[撮合 §8]` `[TDD]` **九条订单簿验收用例全部通过**（退出条件 E3）。

## Phase 4：账户与记账（无杠杆）

- [ ] **M1-T401** `[账户 §1]` 账户实体：`wallet_units`、`position_units`、
      `entry_notional_units`、`reserved_units`、`realized_pnl_units`、`state`。
      **字段全部就位**，保证金逻辑留空（M2 填）。
- [ ] **M1-T402** `[账户 §2.1]` `[TDD]` `entry_notional` 更新：同向加仓 / 反向平仓 /
      **反向翻仓**三条路径；`avg_entry` 向零取整，余数留在 `entry_notional`。
- [ ] **M1-T403** `[账户 §2.2]` `[TDD]` 未实现盈亏与**双口径权益**：
      `equity(mark) = wallet + position × mark − entry_notional`，据此导出
      `risk_equity = equity(risk_mark)` 与 `valuation_equity = equity(valuation_mark)`。
      **两者不得互相替代**：风险公式一律用 `risk_equity`，报告与 PnL 桥接一律用
      `valuation_equity`。M1 虽不做保证金判定，但双口径须在此就位——否则 M2 会在
      单一口径的基础上叠加。
- [ ] **M1-T404** `[ADR-001 §3]` `[TDD]` 手续费：**向上取整**，方向恒不利于代理；
      负 maker 费率（返佣）同样向上取整。费用账户为**有符号**累计量。
- [ ] **M1-T405** `[事件 Schema §4.2.1]` `[TDD]` 账户分录 `postings`：长度恒为 2，
      含 `*_delta` 与 `*_after`。**每次账户变动都由引发它的事件承载**。
- [ ] **M1-T406** `[账户 §2.3]` `[TDD]` **C1/C2 逐事件断言**——整数精确相等，
      不得写成容差断言。违反即测试失败并打印失衡账户。
      逐事件价值断言**必须含 `entry_notional_delta`**（plan §5.2）：
      `Σ(wallet_delta − entry_notional_delta) + 费用 + 风险 = 0`。
      漏掉 `entry_notional` 会把合法的跨价换手判为失败。
- [ ] **M1-T407** `[验收向量]` `[TDD]` **案例 1—5、10 全部通过**（退出条件 E2）。
      案例 2（三代理跨价换手）为必测项，只做案例 1 会误证已推翻的旧等式。
- [ ] **M1-T407b** `[验收向量 7b]` `[TDD]` `reserved_units` 四组场景。**M1 只需算出
      并记录该值**（准入判定桩恒通过），M2 才接入拒绝逻辑——但公式与分录须在 M1 就
      正确，否则 M2 会在错误基础上叠加。
- [ ] **M1-T408** `[指标字典 §5.2]` `[TDD]` PnL 桥接逐事件残差为 0，
      用 `valuation_mark`（**不是** `risk_mark`）。

## Phase 5：制度钩子（接口就位）

- [ ] **M1-T501** `[001 / D-1]` 钩子接口：`validate_order`、`session_state`、
      `settlement_rule`、`margin_rule`、`price_bound`。**调用点必须全部就位**，
      遗漏一个时点会导致整类制度将来无法表达。
- [ ] **M1-T502** `[001 / D-1]` 加密式配置的空实现：24/7、即时结算、无涨跌停、
      无熔断。**钩子只能拒绝或延迟，不能改写订单**。

## Phase 6：确定性与验收

- [ ] **M1-T601** `[代理策略 §10.1—§10.2]` `[P]` RNG：`blake2b` 长度前缀语义键 →
      开区间均匀数。**不用 `SeedSequence`**（NumPy 非标准库）。
      M1 只需均匀分布，其余分布 M2 补。
- [ ] **M1-T602** `[SC-002]` `[TDD]` 确定性：同配置同种子两次运行的事件摘要哈希
      相同（退出条件 E4）。
- [ ] **M1-T603** `[SC-006]` `[TDD]` **独立验证器** `verify`：只读事件日志，
      **不导入 `kernel/` 或 `ledger/`**——复用内核代码就无法证明日志自包含。
      重建账户终态、校验因果链引用完整性、校验 C1/C2。
- [ ] **M1-T604** `[KR-005]` `[TDD]` 导入检查：核心领域层无 NumPy 等第三方导入
      （退出条件 E8）。
- [ ] **M1-T605** `[plan §5.2]` 属性测试：随机订单流（含极端价格、边界数量、自成交、
      跨档）下 C1/C2 恒成立、`queue_key` / `log_key` 各自严格递增、状态机无非法转移。
- [ ] **M1-T606** `[NFR-002]` 覆盖率：订单簿与账本分支覆盖 ≥ 90%（退出条件 E9）。

---

## 退出检查清单

全部勾选后 M1 完成，方可进入 M2：

- [ ] E1 C1/C2 逐事件精确成立（M1-T406）
- [ ] E2 验收向量 1—5、10 通过（M1-T407）
- [ ] E3 撮合九条用例通过（M1-T308）
- [ ] E4 事件摘要哈希稳定（M1-T602）
- [ ] E5 KR-006 单调性断言生效（M1-T202、M1-T203）
- [ ] E6 日志自包含、因果链完整（M1-T603）
- [ ] E7 规范序列化逐字节确定（M1-T104）
- [ ] E8 核心层无第三方导入（M1-T604）
- [ ] E9 分支覆盖 ≥ 90%（M1-T606）

## 遇到合同缺陷时

实现中发现合同不可行或自相矛盾时：

1. **停下，不要绕过**——绕过会使合同失去裁判地位；
2. 记录问题与最小复现；
3. 按 PRD §10 修合同（必要时记 ADR、提升 `schema_version`）；
4. 重算受影响的验收向量期望值；
5. 再继续实现。

四轮复审都是在文档层发现错误的，实现层大概率还会再发现——这是预期之内的，
不是意外。
