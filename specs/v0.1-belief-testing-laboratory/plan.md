...
- L4 只消费事件日志，**不被 L1—L3 引用**（NFR-004、001 / D-7）；
- L2 通过接口与 L1 交互，不直接触碰订单簿内部结构；
- 制度钩子（001 / D-1）由 L1 定义接口、由配置注入实现，**撮合核心不含制度判断**。
...
| 阶段 | 范围 | 退出条件 | 文档 |
|---|---|---|---|
| **0.1.1** | 最小确定性内核（**无杠杆**） | C1/C2 逐事件精确成立；验收案例 1—5、10 与撮合九项用例通过；确定性哈希稳定 | [0.1.1 spec](0.1.1-minimal-kernel/spec.md) / [tasks](0.1.1-minimal-kernel/tasks.md) |
