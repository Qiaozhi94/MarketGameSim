...
- [ ] **T204d** `[事件 Schema §1.5]` `[TDD]` **fail-stop 失败语义**：事务中抛出异常时
...
- [ ] **T204e** `[事件 Schema §6.1/§6.2]` `[TDD]` **三种判别记录**
...
- [ ] **T206b** `[事件 Schema E-002]` `[TDD]` **哈希字段覆盖检查**：对每个事件类型断言
...
- [ ] **T306b** `[撮合 §1.2]` `[事件 Schema §1.5]` `[TDD]` **撮合事务的可见性原子性**：
...
- [ ] E2 账户验收向量 **案例 1—5、10** 通过（T407）
- [ ] E3 订单簿向量 **{OB-1, OB-2, OB-3, OB-4, OB-5, OB-6, OB-7, OB-9a}** 全部通过
